...
import type { Note, NoteContent, ChecklistItem } from "@/services/notes/notesService";
...
export function NoteCard({ note, onPin, onDelete, onUpdate }: Props) {
  ...
  // Narrow content to concrete types based on the discriminant field
  const textContent  = note.type === "text"      ? (note.content as { text: string })           : null;
  const listContent  = note.type === "checklist"  ? (note.content as { items: ChecklistItem[] }) : null;
  const listItems    = listContent?.items ?? [];
